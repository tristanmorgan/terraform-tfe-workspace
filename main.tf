...
resource "tfe_organization_token" "token" {
  ...
  organization = var.tfe_org_id != "" ? var.tfe_org_id : tfe_organization.org[0].id
}
...
resource "tfe_team_token" "producer" {
  ...
  team_id = tfe_team.producer[0].id
}
...
resource "tfe_team_token" "consumer" {
  ...
  team_id = tfe_team.consumer[0].id
}
...
resource "terraform_data" "tfe_oauth_token" {
  triggers_replace = [
    var.tfe_token,
    var.tfe_org_name,
  ]
  ...
  provisioner "local-exec" {
    command = templatefile(
      "${path.module}/tfe-oauth-token.sh.tpl",
      {
        token        = var.tfe_token
...
resource "tfe_workspace" "producer" {
  ...
  depends_on = [terraform_data.tfe_oauth_token]
}
...
resource "tfe_team_access" "producer_team" {
  ...
  team_id      = var.tfe_producer_team_id != "" ? var.tfe_producer_team_id : tfe_team.producer[0].id
...
resource "tfe_team_access" "consumer_team" {
  ...
  team_id      = var.tfe_consumer_team_id != "" ? var.tfe_consumer_team_id : tfe_team.consumer[0].id
  workspace_id = tfe_workspace.producer.id
}
...
resource "tfe_variable" "tfe_organization_token" {
  ...
  value        = tfe_organization_token.token[0].token
  category     = "terraform"
  sensitive    = true
  workspace_id = tfe_workspace.producer.id
}
...
resource "tfe_variable" "tfe_producer_team_token" {
  ...
  value        = tfe_team_token.producer[0].token
  category     = "terraform"
  sensitive    = true
  workspace_id = tfe_workspace.producer.id
}
...
resource "tfe_variable" "tfe_consumer_team_token" {
  ...
  value        = tfe_team_token.consumer[0].token
  category     = "terraform"
  sensitive    = true
  workspace_id = tfe_workspace.producer.id
}
